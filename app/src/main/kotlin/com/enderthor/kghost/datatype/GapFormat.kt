...
/** Feet in a metre — for imperial distance rendering. */
internal const val FEET_PER_METRE = 3.280839895

/**
 * Formats the gap distance in the rider's distance unit: metres (metric) or feet ([imperial]). The
 * value is rounded ONCE in the display unit and both the dead-band and the magnitude derive from that
 * rounded value: an exact 0 renders with NO leading sign ("0 m"/"0 ft"), otherwise an explicit sign
 * so ahead reads positive ("+120 m" / "+390 ft"). The engine already uses positive = ahead.
 */
internal fun fmtDistance(gapDistanceM: Double, imperial: Boolean = false): String {
    ...
    // Round ONCE in the display unit and derive both the dead-band and the magnitude from the same
    // rounded value, so sign and magnitude can never disagree (e.g. "2 m" unsigned while 1.9 m behind).
    val v = (if (imperial) gapDistanceM * FEET_PER_METRE else gapDistanceM).roundToInt()
    val sign = when {
        v == 0 -> ""
        v > 0 -> "+"
        else -> "-"
    }
    return String.format(Locale.US, "%s%d %s", sign, abs(v), if (imperial) "ft" else "m")
}
 ...
 * prolonged GPS loss ([com.enderthor.kghost.engine.GapState.estimated]). Signals "extrapolated,
...
internal fun gapRenderKey(
    ...
    return GapRenderKey(
        ...
        // Quantize in the DISPLAYED unit (feet when imperial), not always metres — otherwise two
        // gaps in the same whole metre render as different whole feet (1 m ≈ 3.28 ft) yet share a
        // key, so the dedup would suppress a redraw the feet readout actually needed.
        distM = if (imperial) {
            (state.gapDistanceM * FEET_PER_METRE).toQuantOrSentinel()
        } else {
            state.gapDistanceM.toQuantOrSentinel()
        },
        gapDisplay = gapDisplay,
        dark = dark,
        imperial = imperial,
